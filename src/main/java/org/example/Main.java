...
public class Main {
  public static void main(String[] args) throws IOException {
    ...
    try (var generator = StandaloneRefinery.getGeneratorFactory().createGenerator(problem)) {
      ...
      var tRelation = trace.getPartialRelation("N::t");
      var tInterpretation = generator.getPartialInterpretation(tRelation);

      System.out.println("=== Quantum Minor Embedding Result ===");
      var cursor = tInterpretation.getAll();
      while (cursor.move()) {
        if (cursor.getValue() == TruthValue.TRUE) {
          Tuple tuple = cursor.getKey();
          System.out.println(tuple.toString());

          //System.out.printf("Physical Qubit: %-5s -> Logical Qubit: %s%n", nodeName, typeName);
        }
      }
    }
  }
